...
if (jQuery.fn.summernote && jQuery('.summernote').length) {
  jQuery('.summernote').summernote({
	...
	jQuery(document).on('click', '.coursemoduleadd', function(){
		...
		jQuery('div#' + safeModule).find('div.coursemoduledetails:last').after(html);
		
		if(jQuery('div#' + safeModule).find('.coursemoduletitle').length >= 2){
			if(jQuery('div#' + safeModule).find('.coursemoduletitle').length == 2){
	...
	jQuery(document).on('click', '.coursemoduleremove', function(){
		...
		.then((willDelete) => {
			if(willDelete){
				...
				if(jQuery('div#' + safeModule).find('.coursemoduletitle').length == 1){
					jQuery(document).find('div#' + safeModule).find('div.coursemoduletitle:eq(0)').find('div.coursemoduleaction').html('<a href="javascript:;" class="text-warning coursemoduleadd" aria-label="Add new module" data-bs-original-title="Add new module"><i class="bi bi-plus-circle fs-4"></i></a>');
				}else if(jQuery('div#' + safeModule).find('.coursemoduletitle').length == 2){
					jQuery(document).find('div#' + safeModule).find('div.coursemoduletitle:eq(0)').find('div.coursemoduleaction').html('<a href="javascript:;" class="text-danger coursemoduleremove" aria-label="Delete module" data-bs-original-title="Delete module"><i class="bi bi-trash-fill fs-4"></i></a>');
...
jQuery(document).on('click', '.consultingremove', function(){
 ...
 $form.find('.summernote').each(function () {
  ...
  jQuery.ajax({
	...
	if ($('.author-select').length) {
  $('.author-select').select2({
    theme: 'bootstrap4',
    width: '100%',
    tags: true,                 // ✅ allow new author
    placeholder: 'Add New Author',
    allowClear: true,
    tokenSeparators: [',']       // optional (type comma to create)
  });
}
