    ...
    <div class="ps-3 pl-0i">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 p-0">
          ...
          <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('galleries.index', [$media, $tag]) }}">Training videos</a></li>
          <li class="breadcrumb-item active" aria-current="page">Training video details</li>
        </ol>
      </nav>
    </div>
  </div>
                ...
                <div class="form-group">
                  ...
                  <select name="video_year" id="videoyear" class="form-control" data-placeholder="Video year">
                    ...
                    @for($year = 2020; $year <= Carbon\Carbon::now()->year; $year++)
                      <option value="{{ $year }}" {{ ($gallery->video_year == $year) ? 'selected' : '' }}>{{ $year }}</option>
                    @endfor
                  </select>
