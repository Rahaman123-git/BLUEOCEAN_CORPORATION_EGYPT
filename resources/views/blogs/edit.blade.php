...
@section('content')
        ...
        <form name="blogdetailsform"
          ...
          <div class="card-body">
            ...
            <div class="row mb-3">
              <div class="col-md-12">
                <div class="form-group">
                  <label class="control-label">Introduction details</label>
  ...
  <div class="col-md-6">
    <div class="form-group">
      ...
      @php
  $selectedAuthor = old('created_by', $blog->created_by ?? '');
@endphp

<select name="created_by" id="created_by" class="form-control author-select" data-placeholder="Add New Author">
  {{-- IMPORTANT: keep empty option and auto-select it when blank --}}
  <option value="" {{ empty($selectedAuthor) ? 'selected' : '' }}></option>

  @foreach($authors ?? [] as $author)
    <option value="{{ $author }}" {{ $author === $selectedAuthor ? 'selected' : '' }}>
      {{ $author }}
    </option>
  @endforeach
</select>

<small class="text-muted">Type a name and press Enter to add new author.</small>
<div id="created_by" class="invalid-feedback"></div>
    ...
    <div class="form-group">
      <label class="control-label">
        Published Date 
		 ...
		 <input type="text" name="published_date" id="published_date" class="form-control datepicker" placeholder="published date" value="{{ old('published_date', $blog->published_date ? \Carbon\Carbon::parse($blog->published_date)->format('d.m.Y') : '') }}"
>

      <div id="published_date" class="invalid-feedback"></div>
    </div>
  </div>
