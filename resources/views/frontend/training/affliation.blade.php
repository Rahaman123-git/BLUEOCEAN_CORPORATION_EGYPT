...
@section('title', ' IPSCMI, CILT, CIPS, ASCM, APICS, PMI, IATA & ACI - Blue Ocean Academy Affliations')
    ...
    <div class="container">
      <div class="flex justify-between items-center aff-title-box">
        ...
        <div class="back-button">
          <a href="{{ route('training.affliations.expanded') }}" class="flex text-primary items-center"> <img src="{{ asset('assets/frontend/images/arrow-left-blue.svg') }}" width="30"/>View All Affilations </a>
        </div>
      </div> 
    </div>
  </section>

  <section class="affiliation-section py-10">
    <div class="container">
      <div class="grid grid-cols-1 lg:px-5">
        @if($affiliations)
          <div class="affiliate-slider">
            @foreach($affiliations as $aKey => $affiliation)
              <div class="item">
                <div class="affiliate-logo">
                  <button data-tab="tab-{{ $aKey }}" class="tab-button {{ !$aKey ? 'active' : '' }}">
                    <img src="{{ asset('storage/courses/affiliations/'.$affiliation->image) }}" alt="" />
                  </button>
                </div>
              </div>
            @endforeach
    ...
    <div class="container">
      ...
      <div class="w-full max-w-md mx-auto mt-10">
        <div class="mt-4">
          @if($affiliations)
            @foreach($affiliations as $aKey => $affiliation)
              <div id="tab-{{ $aKey }}" class="tab-panel {{ !$aKey ? 'active' : 'hidden' }} p-4 bg-white">
